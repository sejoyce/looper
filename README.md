...
9. The winning loop is drawn on a Leaflet map (green = trail, blue = street) with small arrow markers along the route showing which direction to run it, plus distance, trail %, traffic lights, turn count, and elevation gain, and a turn-by-turn breakdown, in the sidebar. If it couldn't get within tolerance, had to fall back to laps, or couldn't fully satisfy an elevation preference, the sidebar says so plainly.
