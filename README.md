...
11. The winning loop is drawn on a Leaflet map (using a muted Esri Light Gray basemap so the route stands out) with small arrow markers along it (colored to match trail/street, using a colorblind-safe palette) showing which direction to run it, plus distance, trail %, traffic lights, turn count, and elevation gain, and a turn-by-turn breakdown, in the sidebar. Every generated loop is kept as a small thumbnail in a history strip so you can flip back to compare earlier options within the session. A loading overlay appears on the map itself (not just sidebar text) while a route is being found, since that can take a few seconds on denser street networks.
...
- **Colors**: trail/street/arrows use a colorblind-safe palette (Okabe-Ito: bluish-green `#009E73` for trail, blue `#0072B2` for street), applied to the rest of the app's UI too (brighter than the map colors, but the same hues) so the whole interface feels visually consistent. The basemap is Esri's "World Light Gray Canvas" — a muted, mostly-monochrome map, free to use without an API key — specifically so the route is the most visually prominent thing on screen rather than competing with a busy default street-map style. (CARTO's equivalent basemap now requires a registered API key, which is why this app doesn't use it.) Using a third-party basemap tile provider means the map depends on Esri's tile service being reachable in addition to Nominatim/Overpass/Open-Elevation.
- **One button, two behaviors**: the form's single submit button reads "Find my loop" or "Try a different loop" depending on whether a route is already showing, and its color changes to match (orange vs. green). Clicking it regenerates on the same street data if the address, mileage, and elevation preferences are unchanged since the last generation; otherwise it runs a fresh search (new geocode + street data fetch), even if that fresh search happens to be triggered by the "regenerate"-labeled state of the button from a moment ago.
